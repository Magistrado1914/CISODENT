<?php
class Dialogflow {
    private $usuario_recibido;
    private $pass_recibido;
    private $inputJSON;
    private $input;
    private $respuesta;
    private $respuesta_texto;
    private $respuesta_cards;
    private $respuesta_images;
    
    public function __construct(){
        $this->respuesta = "";
        $this->respuesta_texto = "";
        $this->respuesta_cards[] = array();
        $this->respuesta_images[] = array();
    ...
    */
    ...
    public function credenciales($usuario, $pass)
    {
        echo $usuario;
        $this->usuario_recibido = $_SERVER['PHP_AUTH_USER'];
        $this->pass_recibido = $_SERVER['PHP_AUTH_PW'];
        if (($usuario != $usuario) OR ($pass != $pass)) {
    ...
    function debug()
    {
        $inputJSON = $this->inputJSON = file_get_contents('php://input');
        $input = $this->input = json_decode($inputJSON, TRUE);
        $json_string = json_encode($this->input, JSON_PRETTY_PRINT);
        file_put_contents('json.js', $json_string);
        file_put_contents('credenciales_recibidas.js', "Usuario: " . $this->usuario_recibido . " Pass: " . $this->pass_recibido);
    ...
    */
    ...
    public function enviar_texto($texto)
    {
